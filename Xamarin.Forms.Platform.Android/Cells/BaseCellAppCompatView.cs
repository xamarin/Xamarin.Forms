...
{
	public class BaseCellAppCompatView : LinearLayout, INativeElementView
	{
		...
		readonly AppCompatImageView _imageView;
		...
		public BaseCellAppCompatView(Context context, Cell cell) : base(context)
		{
			_cell = cell;
		...
		async void UpdateBitmap(ImageSource source, ImageSource previousSource = null)
		{
			...
			{
				Log.Warning(nameof(BaseCellAppCompatView), "Error loading image: {0}", ex);
			}
		}
	}
}