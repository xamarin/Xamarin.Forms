...
#pragma warning restore 0618
		...
		[EditorBrowsable(EditorBrowsableState.Never)]
		public IDictionary<int, List<VisualElement>> GetTabIndexesOnParentPage(out int countChildrensWithTabStopWithoutThis)
		{
			...
			if (!childrensWithTabStop.Contains(this))
				return null;
		...
		public VisualElement FindNextElement(bool forwardDirection, IDictionary<int, List<VisualElement>> tabIndexes, ref int tabIndex)
		{
			...
			if (!forwardDirection)
			{
				...
				else // search prev element in prev TabIndex group
				{
					...
					result = tabIndexes[tabIndex][0];
				}
			}
			else // Forward
			{
				// search next element in same TabIndex group
				var nextSubIndex = tabGroup.IndexOf(this) + 1;
				if (nextSubIndex > 0 && nextSubIndex < tabGroup.Count)
				{
					result = tabGroup[nextSubIndex];
				}
