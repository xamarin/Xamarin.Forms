...
namespace Xamarin.Platform.Tizen
{
	public static class ThemeManager
	{
		...
		public static Button SetWatchPopupRightStyle(this Button button)
		{
			...
			{
				Log.Error($"ToWatchPopupRightStyleButton is only supported on wearable profile : {0}", DeviceInfo.GetDeviceType());
				return button;
			}
		...
		public static Toolbar SetTVTabBarWithTitleStyle(this Toolbar toolbar)
		{
			if (!DeviceInfo.IsTV)
			{
				Log.Error($"ToWatchPopupRightStyleButton is only supported on TV profile : {0}", DeviceInfo.GetDeviceType());
				return toolbar;
			}
