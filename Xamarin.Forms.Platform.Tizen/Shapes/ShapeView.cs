...
namespace Xamarin.Forms.Platform.Tizen
{
    public class ShapeView : Canvas, IMeasurable
    {
            ...
            if (_fill != null)
            {
                ...
                if (_fill is GradientBrush fillGradientBrush)
                {
                    if (fillGradientBrush is LinearGradientBrush linearGradientBrush)
                        fillShader = CreateLinearGradient(linearGradientBrush, fillBounds);

                    if (fillGradientBrush is RadialGradientBrush radialGradientBrush)
                        fillShader = CreateRadialGradient(radialGradientBrush, fillBounds);

                    _skPaint.Shader = fillShader;
                }
                else
                {
                    SKColor fillColor = XColor.Default.ToNative().ToSKColor();
                    if (_fill is SolidColorBrush solidColorBrush && solidColorBrush.Color != XColor.Default)
                        fillColor = solidColorBrush.Color.ToNative().ToSKColor();

                    _skPaint.Color = fillColor;
                }
            ...
            if (_stroke != null)
            {
                ...
                if (_stroke is GradientBrush strokeGradientBrush)
                {
                    ...
                    if (strokeGradientBrush is LinearGradientBrush linearGradientBrush)
                        strokeShader = CreateLinearGradient(linearGradientBrush, strokeBounds);

                    if (strokeGradientBrush is RadialGradientBrush radialGradientBrush)
                        strokeShader = CreateRadialGradient(radialGradientBrush, strokeBounds);

                    _skPaint.Shader = strokeShader;
                }
                else
                {
                    SKColor strokeColor = XColor.Default.ToNative().ToSKColor();
                    if (_stroke is SolidColorBrush solidColorBrush && solidColorBrush.Color != XColor.Default)
                        strokeColor = solidColorBrush.Color.ToNative().ToSKColor();

                    _skPaint.Color = strokeColor;
                }
        ...
        SKShader CreateLinearGradient(LinearGradientBrush linearGradientBrush, SKRect pathBounds)
        {
            if (_skPath == null)
                return null;

            var startPoint = new SKPoint(pathBounds.Left + (float)linearGradientBrush.StartPoint.X * pathBounds.Width,
                pathBounds.Top + (float)linearGradientBrush.StartPoint.Y * pathBounds.Height);
            var endPoint = new SKPoint(pathBounds.Left + (float)linearGradientBrush.EndPoint.X * pathBounds.Width,
                pathBounds.Top + (float)linearGradientBrush.EndPoint.Y * pathBounds.Height);
        ...
        SKShader CreateRadialGradient(RadialGradientBrush radialGradientBrush, SKRect pathBounds)
        {
            var center = new SKPoint((float)radialGradientBrush.Center.X * pathBounds.Width + pathBounds.Left,
                (float)radialGradientBrush.Center.Y * pathBounds.Height + pathBounds.Top);
