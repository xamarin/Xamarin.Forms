...
//					IL_0016:  pop
		...
		static Instruction PushParsedEnum(TypeReference enumRef, string value, IXmlLineInfo lineInfo)
		{
			...
			foreach (var v in value.Split(',')) {
				foreach (var field in enumDef.Fields) {
					...
					if (field.Name == v.Trim()) {
						...
						case "System.SByte":
							if (found)
								throw new XFException(XFException.Ecode.MultiEnumToSbyte, lineInfo);
			...
			if (!found)
				throw new XFException(XFException.Ecode.SomethingNotFound, lineInfo, enumRef.Name, value);
