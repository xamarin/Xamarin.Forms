...
& dotnet pack $PSScriptRoot\..\src\Controls\src\Build.Tasks\Controls.Build.Tasks-net6.csproj -c:$configuration -p:PackageVersion=$packageVersion -p:IncludeSymbols=true -p:SymbolPackageFormat=snupkg
& dotnet pack $PSScriptRoot\..\src\SingleProject\Resizetizer\src\Resizetizer.csproj -c:$configuration -p:PackageVersion=$packageVersion -p:IncludeSymbols=true -p:SymbolPackageFormat=snupkg

& dotnet pack $PSScriptRoot\..\src\Package\Package-net6.csproj -c:$configuration -p:PackageVersion=$packageVersion -p:IncludeSymbols=true -p:SymbolPackageFormat=snupkg