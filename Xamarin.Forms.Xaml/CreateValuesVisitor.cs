	...
	class CreateValuesVisitor : IXamlNodeVisitor
	{
		...
		public void Visit(ElementNode node, INode parentNode)
		{
			...
			if (IsXaml2009LanguagePrimitive(node))
				...
				throw new XamlParseException("XF0002", node, ctorargname, type.FullName);
			}
			else {
				...
				try {
					...
					throw new XamlParseException("XF0001", node, mme, mme.Message);
				}
			}
			...
			if (value is IMarkupExtension markup && (value is TypeExtension || value is StaticExtension || value is ArrayExtension)) {
				...
				try {
					...
					var xamlpe = e as XamlParseException ?? new XamlParseException("XF0058", serviceProvider.GetLineInfo(), e, "Markup");
					if (Context.ExceptionHandler != null) {
