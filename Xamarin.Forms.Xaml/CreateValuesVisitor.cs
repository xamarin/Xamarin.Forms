		...
		public object[] CreateArgumentsArray(IElementNode enode, ConstructorInfo ctorInfo)
		{
			...
			for (var i = 0; i < n; i++) {
				...
				if (!enode.Properties.TryGetValue(name, out INode node))
					throw new XamlParseException($"The Property {propname} is required to create a {ctorInfo.DeclaringType.FullName} object.", enode as IXmlLineInfo, errorCode: "CSXF1778");
