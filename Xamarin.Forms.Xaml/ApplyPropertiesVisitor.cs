	...
	class ApplyPropertiesVisitor : IXamlNodeVisitor
	{
		...
		public void Visit(ElementNode node, INode parentNode)
		{
			...
			if (propertyName != XmlName.Empty || TryGetPropertyName(node, parentNode, out propertyName)) {
				...
				xpe = xpe ?? new XamlParseException("XF0004", node, ((IElementNode)parentNode).XmlType.Name, "ContentPropertyAttribute");
				...
				if (collection == null)
					xpe = new XamlParseException("XF0002", node, parentList.XmlName.LocalName, "IEnumerable");
				...
				xpe = xpe ?? new XamlParseException("XF0003", node, parentList.XmlName.LocalName);
				if (Context.ExceptionHandler != null)
					Context.ExceptionHandler(xpe);
				else
					throw xpe;
		...
		public static void SetPropertyValue(object xamlelement, XmlName propertyName, object value, object rootElement, INode node, HydrationContext context, IXmlLineInfo lineInfo)
		{
			...
			xpe = xpe ?? new XamlParseException("XF0006", lineInfo, localName);
			if (context.ExceptionHandler != null)
				context.ExceptionHandler(xpe);
			else
				throw xpe;
		...
		static bool TryConnectEvent(object element, string localName, bool attached, object value, object rootElement, IXmlLineInfo lineInfo, out Exception exception)
		{
			...
			exception = new XamlParseException("XF0046", lineInfo, value.ToString(), rootElement.GetType().ToString());
			return false;
		}
		...
		static bool TrySetDynamicResource(object element, BindableProperty property, object value, IXmlLineInfo lineInfo, out Exception exception)
		{
			...
			if (bindable == null) {
				exception = new XamlParseException("XF0044", lineInfo, elementType.Name, "BindableObject");
				return false;
			}
			...
			if (attached)
				minforetriever = () =>
				{
					try {
						...
						throw new XamlParseException("CS0102", lineInfo, innerException: e, property.DeclaringType.ToString(), "Get" + property.PropertyName);
					}
				};
			else
				minforetriever = () =>
				{
					try {
						...
						throw new XamlParseException("CS0102", lineInfo, innerException: e, property.DeclaringType.ToString(), property.PropertyName);
					}
				};
		...
		static bool TryGetProperty(object element, string localName, out object value, IXmlLineInfo lineInfo, HydrationContext context, out Exception exception, out object targetProperty)
		{
			...
			while (elementType != null && propertyInfo == null) {
				try {
					...
					throw new XamlParseException("CS0102", lineInfo, innerException: e, elementType.ToString(), localName);
				}
		...
		static bool TryAddToResourceDictionary(ResourceDictionary resourceDictionary, object value, string xKey, IXmlLineInfo lineInfo, out Exception exception)
		{
			...
			if (xKey != null)
				...
				exception = new XamlParseException("XF0026", lineInfo, nameof(ResourceDictionary));
				return false;
			}
			return true;
		}
